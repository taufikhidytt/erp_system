...
$.extend(true, $.fn.dataTable.defaults, {
	...
	drawCallback: function (settings) {
		...
		var info = api.page.info();
		var oldPage =
			settings._oldPage !== undefined ? settings._oldPage : info.page;
...
$(document).ready(function () {
	$(document).on("init.dt", function (e, settings) {
		if (
			e.namespace !== "dt" ||
			settings.oInit.showScrollToggle === false ||
		...
		$toggleContainer.find(".dt-scroll-checkbox").on("change", function () {
			$(this)
				.closest(".dt-autoscroll-toggle")
				.find(".dt-autoscroll-label")
				.text("Scroll " + (this.checked ? "On" : "Off"));
	...
	$(document).on("wheel", ".dataTables_scrollBody", function (e) {
		...
		if (
			!dt ||
			dt.settings()[0].oInit.autoScrollPage !== true ||
			$table.data("isPageChanging")
		)
			return;

		const scrollTop = $scrollBody.scrollTop();
		const isBottom =
			Math.ceil(scrollTop + $scrollBody.innerHeight()) >=
			$scrollBody[0].scrollHeight;
		const isTop = scrollTop === 0;
		const info = dt.page.info();

		let action = null;

		if (oe.deltaY > 0 && isBottom && info.page < info.pages - 1)
			action = "next";
				...
				setTimeout(
					function () {
						$scrollBody.scrollTop(
							action === "previous" ? $scrollBody[0].scrollHeight : 0,
						);
...
function syncTableHeader(settings) {
	...
	var isEmpty =
		$bodyTds.length === 0 ||
		($bodyTds.length === 1 &&
			$firstRow.hasClass("odd") &&
			$bodyTds.first().attr("colspan"));
...
$(document).on("keydown", function (e) {
	if (e.which === 32) {
		const focusedCell = $(document).find(
			".dataTables_wrapper table tbody td.focus",
		);
	...
	$("#theme-toggle-btn").on("click", function () {
		...
		if (currentTheme === "light") {
			$("#bootstrap-style").attr(
				"href",
				baseUrl + "assets/admin/css/bootstrap.min.css",
			);
			...
			$("#bootstrap-style").attr(
				"href",
				baseUrl + "assets/admin/css/bootstrap-dark.min.css",
			);
			$("#app-style").attr(
				"href",
				baseUrl + "assets/admin/css/app-dark.min.css",
			);
...
$(function () {
	...
	let pending = {
		...
		datetime:
			getCookie("app-show-datetime") !== null
				? getCookie("app-show-datetime")
				: "1",
	};
