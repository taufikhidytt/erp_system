...
class Po_kny extends Back_Controller
{
    ...
    public function add()
    {
        try {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $post['supplier'] = explode('_', $post['supplier'])[0];
    ...
    public function get_log_info()
    {
        ...
        $config = [
            'queries' => [
                [
                    ...
                    'where'  => "a.TABLE_NAME = 'INVOICE' AND a.ID = " . $id,
                ],
                [
                    'select' => 'a.LAST_UPDATE_DATE, u.ERP_USER_NAME, a.TRANSAKSI, a.NOTE',
                    'table'  => 'erp_log_edit a',
                    'join'   => ['erp_user u', 'a.LAST_UPDATE_BY = u.ERP_USER_ID'],
                    'where'  => "a.TABLE_NAME = 'INVOICE_DETAIL' AND a.ORDER_ID = " . $id,
                ],
            ],
        ...
        $info_header    = $this->po_kny->get_log_user($id);
        $result['header'] = [
