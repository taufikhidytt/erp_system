...
class Out_so extends Back_Controller
{
    ...
    {
        try {
            $data['title'] = 'Info OutStd (MR-SO-PO)';
            $data['breadcrumb'] = 'Info OutStd (MR-SO-PO)';
    ...
    public function export()
    {
        ...
        header('Content-Disposition: attachment;filename="Info OutStd (MR-SO).xlsx"');
    ...
    public function get_data_mr_po()
    {
        $list = $this->out_so->get_datatables_mr_po();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->out_so->count_all_mr_po(),
            "recordsFiltered" => $this->out_so->count_filtered_mr_po(),
            "data" => $data,
        );
        echo json_encode($output);
    }

    public function export_mr_po()
    {
        $params = $this->input->get();
        $_POST_backup = $_POST;
        $_POST = $params; // pakai parameter dari DataTables

        $list = $this->out_so->get_datatables_export_mr_po();
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Info OutStd (MR-PO).xlsx"');
