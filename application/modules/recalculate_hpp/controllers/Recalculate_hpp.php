...
class Recalculate_hpp extends Back_Controller
{
    ...
    public function index()
    {
        try {
            ...
            $data['active_job'] = $this->recalculate_hpp->get_active_job();
    ...
    public function start_sync()
    {
        ...
        $active = $this->recalculate_hpp->get_active_job();
        if ($active) {
            echo json_encode(['status' => false, 'message' => 'Masih ada proses sinkronisasi yang berjalan.']);
            return;
        }

        $job_id = $this->recalculate_hpp->create_job('RECALCULATE_HPP_ITEM');
        ...
        $app_path = FCPATH . 'app';
        $cmd = "php " . escapeshellarg($app_path) . " recalculate_hpp/sync cli_process " . escapeshellarg($job_id) . " " . escapeshellarg($encoded_db);
    ...
    {
        $data['history']    = $this->recalculate_hpp->get_history();
