<?php foreach ($account as $k => $v) {
    ...
    ?>
    <li>
        <div class="tree-node<?= (($n==1 && !$active) || $active == $v['ACCOUNT_ID'])?" active-node":'' ?><?= $active_flag=='N'?" node-inactive":'' ?>" data-id="<?= base64url_encode($this->encrypt->encode($v['ACCOUNT_ID'])) ?>" data-code="<?= $v['ACCOUNT_CODE'] ?>" data-name="<?= $v['ACCOUNT_NAME'] ?>" data-type="<?= $v['ACCOUNT_TYPE'] ?>" data-type_id="<?= $v['ACCOUNT_TYPE_ID'] ?>" data-program_code="<?= $v['PROGRAM_CODE1'] ?>" data-active="<?= $active_flag ?>" data-level="<?= $level ?>" data-kata="<?= $v['KATA'] ?>" data-mata_uang="<?= $v['MATA_UANG_CODE'] ?>" data-mata_uang_id="<?= $v['MATA_UANG_ID'] ?>">
