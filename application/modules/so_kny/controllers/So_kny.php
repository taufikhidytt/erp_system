...
class So_kny extends Back_Controller
{
    ...
    public function get_info($id)
    {
        ...
        $params = [
            'table' => 'so_detail b',
            'select' => [
                'b.SO_DETAIL_ID, COALESCE(i.PART_NUMBER, i.ITEM_DESCRIPTION) Nama_Item, i.ITEM_CODE Kode_Item, b.ENTERED_UOM Satuan, b.ENTERED_QTY SO',
    ...
    public function get_log_info()
    {
        ...
        $config = [
            'queries' => [
                [
                    ...
                    'where'  => "a.TABLE_NAME = 'SO' AND a.ID = " . $id,
                ],
                [
                    'select' => 'a.LAST_UPDATE_DATE, u.ERP_USER_NAME, a.TRANSAKSI, a.NOTE',
                    'table'  => 'erp_log_edit a',
                    'join'   => ['erp_user u', 'a.LAST_UPDATE_BY = u.ERP_USER_ID'],
                    'where'  => "a.TABLE_NAME = 'SO_DETAIL' AND a.ORDER_ID = " . $id,
                ],
            ],
        ...
        $info_header    = $this->so_kny->get_log_user($id);
        $result['header'] = [
