                    ...
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item">
                                <a href="<?= base_url('do_kny') ?>" class="text-decoration-underline"><?= $access['PROMPT'] ?></a>
                            </li>
                                    ...
                                    <div class="col-lg-6 col-md-12 col-sm-12">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="hidden" name="location_id" id="location_id" class="form-control" readonly value="<?= $this->input->post('location_id'); ?>">
                                            </div>
                                            <div class="text-danger"><?= form_error('location') ?></div>
                                        </div>
                                        <div class="mb-3">
                                            <textarea name="address" id="address" class="form-control" disabled><?= $this->input->post('address'); ?></textarea>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-md-12 col-sm-12">
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="po_customer" id="po_customer" class="form-control" placeholder="Enter PO Customer" value="<?= $this->input->post('po_customer'); ?>" readonly>
                                            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (oldDetail && oldDetail.kode_item && Array.isArray(oldDetail.kode_item)) {
            ...
            if (row.child.isShown()) {
                ...
                var childTable = $('#' + $.escapeSelector(childTableId)).DataTable({
                    ...
                    "initComplete": function(settings, json) {
                        ...
                        // Akses flag dari currentTr data
                        ...
                        // Jangan disable child checkbox di initComplete
                        // Biarkan enabled agar bisa di-check/uncheck
                    },
                    "drawCallback": function() {
                        var childTableElem = $('#' + $.escapeSelector(childTableId));

                        // Hide row child checkbox yang sudah ada di table detail
                        ...
                        childTableElem.find('tbody .childCheckbox').each(function() {
                            ...
                            if (soDetailId && existingSoDetailId.has(String(soDetailId))) {
                                // Hide entire row, bukan hanya disable checkbox
                                row.hide();
                            } else {
                                // Show row jika tidak ada di table detail
                                row.show();
                        ...
                        // Jika semua child sudah di table detail (hidden) DAN parent tidak ter-check, hide parent row
                        if (allChildHidden && !hasVisibleChild) {
                            // Find parent row and check if it's checked
                            var parentRow = $('#table-item tbody tr').filter(function() {
                                var row = tableItem.row($(this));
                                var rowData = row.data();
                                return rowData && rowData[10] == so_id;
                            });

                            var parentCheckbox = parentRow.find('.chkRow');
                            if (!parentCheckbox.prop('checked')) {
                                parentRow.hide();
                            }
                        }

                        // Update checkAllChild checkbox state - hanya hitung child yang visible (tidak di-hide)
                        var visibleChild = childTableElem.find('tbody tr:visible .childCheckbox');
                        var checkedVisibleChild = childTableElem.find('tbody tr:visible .childCheckbox:checked');
                        $('#checkAllChild_' + so_id).prop('checked', visibleChild.length > 0 && visibleChild.length === checkedVisibleChild.length);
                    }
                });
            }
        });
        ...
        $(document).on('change', '.chkRow', function() {
            ...
            if (currentCheckbox.prop('checked')) {
                ...
                // Enable child checkbox dari parent yang ter-check
                var childTableCurrent = $('#child-' + currentSoId);
                if (childTableCurrent.length) {
                    childTableCurrent.find('tbody .childCheckbox, #checkAllChild_' + currentSoId).prop('disabled', false);
                }
                ...
                if (!row.child.isShown()) {
                    ...
                    // Row sudah expanded, check semua child sekarang
        ...
        // Hapus data dari table detail saat parent di-uncheck
        $(document).on('change', '.chkRow', function() {
            var currentCheckbox = $(this);
            var currentRow = currentCheckbox.closest('tr');
            var row = tableItem.row(currentRow);
            ...
            // Jika parent di-uncheck, hapus data dari table detail
            if (!currentCheckbox.prop('checked')) {
                var noTransaksi = currentCheckbox.data('no_transaksi');
                if (noTransaksi) {
                    // Hapus rows dari table detail yang memiliki no_transaksi ini
                    var rowsToRemove = [];
                    var soDetailIdsToRemove = [];

                    tableDetail.rows().every(function() {
                        var node = this.node();
                        var detailNoTransaksi = $(node).find('input[name="detail[no_transaksi][]"]').val();
                        var detailSoDetailId = $(node).find('input[name="detail[so_detail_id][]"]').val();

                        if (detailNoTransaksi == noTransaksi) {
                            rowsToRemove.push(this);
                            if (detailSoDetailId) {
                                soDetailIdsToRemove.push(String(detailSoDetailId));
                            }
                        }
                    });
                    ...
                    // Hapus dari global variable
                    soDetailIdsToRemove.forEach(function(id) {
                        existingSoDetailId.delete(id);
                    });
        ...
        $("#btnSubmit").on("click", function(e) {
            ...
            tableDetail.rows().every(function() {
                ...
                let noTransaksi = $(node)
                    .find('input[name="detail[no_transaksi][]"]')
                    .val();
            ...
            // Jika ada parent baru yang berbeda dengan yang sudah ada di table detail,
            // hapus semua data dari parent lama
            let hasDifferentParent = false;
            newNoTransaksiSet.forEach(function(newNoTrans) {
                // Jika no_transaksi baru tidak ada di table detail, berarti ini parent berbeda
                // dan table detail sudah ada isinya dari parent lain
                if (existingNoTransaksi.size > 0 && !existingNoTransaksi.has(newNoTrans)) {
            ...
            // 2. Table detail sudah ada isinya
            if (hasDifferentParent && existingNoTransaksi.size > 0) {
            ...
            $('.childCheckbox:checked').each(function() {
                ...
                if (so_detail_id && existingSoDetailId.has(String(so_detail_id))) {
                    childCheckbox.prop("checked", false).prop("disabled", true);
                    return;
                }
                ...
                let rowNode = tableDetail.row.add([
                    ...
                    <input type="hidden" name="detail[berat][]" value="${berat}">
                    <input type="hidden" name="detail[balance][]" value="${sisa}">
                    <input type="hidden" name="detail[po_no][]" value="${po_no}">
            ...
            if (rowsAdded) {
                ...
                // Uncheck dan disable child checkbox yang sudah dipindahkan
                $('.childCheckbox:checked').each(function() {
