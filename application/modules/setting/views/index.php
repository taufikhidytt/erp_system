...
<script>
    $(document).ready(function() {
        ...
        const editor = InlineEditor.init({
            ...
            urls  : {
                save : '<?= site_url('setting/save') ?>',
            },
            fields: [
                { field: 'note', type: 'text', maxlength: 18, required: true, label: 'Note', editable: false,
                    attrs: { 
                        oninput: "let p = this.selectionStart; this.value = this.value.toUpperCase(); this.setSelectionRange(p, p);"
                    }
                },
                { field: 'account', type: 'select2',required: true, label: 'Account', 
                    select2: {
                        url         : '/setting/get_account',
                        placeholder : 'Pilih Account',
                        labelFn: row => row.COA_NAME,
                    },
                    attrs: {
