            ...
            <div class="col-12">
                <div class="card rounded-3 p-4" id="card-import-main">
                    ...
                    <div id="state-idle">
                        ...
                        <!-- Upload area — tampil setelah dipilih -->
                            ...
                            <?php if ($period->OPEN_FLAG === 'Y'): ?>
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted small mb-4">
                                        Download template terlebih dahulu, isi data <?= strtolower($access['PROMPT']) ?> mulai dari <strong>baris ke-7</strong>,
                        ...
                        <div id="upload-placeholder" class="text-center py-4 text-muted">
                            ...
                            <p class="small mb-0">Pilih periode terlebih dahulu untuk melanjutkan import <?= strtolower($access['PROMPT']) ?>.</p>
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        const URLS = {
            history       : '<?= base_url("gl_balance/import_history") ?>',
            upload        : '<?= base_url("gl_balance/upload") ?>',
            get_status    : '<?= base_url("gl_balance/get_status") ?>',
            finalize      : '<?= base_url("gl_balance/finalize_import") ?>',
            download_failed: '<?= base_url("gl_balance/download_failed/") ?>',
            cancel        : '<?= base_url("gl_balance/import_cancel/") ?>',
        };
        ...
        // Update URL template period
        const period  = $('#period').val();
        const baseUrl = '<?= base_url($access['url'] . '/template_import') ?>';
        $('#btn-download-template').attr('href', baseUrl + '?&p=' + encodeURIComponent(period));
