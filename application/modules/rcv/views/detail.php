                    ...
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item">
                                <a href="<?= base_url('rcv') ?>" class="text-decoration-underline"><?= $access['PROMPT'] ?></a>
                            </li>
                        ...
                        <form action="" method="post" id="myForm">
                            <div class="row mb-2">
                                <div class="col-lg-6 col-md-6 col-sm-12 d-flex align-items-center gap-2 label-status">
                                    <h5 id="statusTagId" style="width: 100px;"></h5>
                                    <h5 style="width: 100px;" id="readonlyTagId"></h5>
                                </div>
                                    ...
                                    <div class="tab-content py-3 text-muted">
                                                            ...
                                                            foreach ($dataDetail->result() as $dd): ?>
                                                                ...
                                                                <tr class="tr-height-30">
                                                                    ...
                                                                    <td class="ellipsis text-end">
                                                                        ...
                                                                        <input type="text" class="form-control form-control-sm qty auto-width edit-mode qty-edit text-end input-number d-none enter-as-tab w-100" min="0" step="any" name="detail[jumlah][]" data-balance="<?= ($balance == 0) ? '0' : rtrim(rtrim((string)$balance, '0'), '.') ?>" data-tag_detail_id="<?= $this->encrypt->encode($dd->TAG_DETAIL_ID) ?>" data-value_old="<?= ($dd->ENTERED_QTY == 0) ? '0' : rtrim(rtrim((string)$dd->ENTERED_QTY, '0'), '.') ?>" value="<?= ($dd->ENTERED_QTY == 0) ? '0' : number_format(rtrim(rtrim((string)$dd->ENTERED_QTY, '0'), '.'), 2, '.', ',') ?>">
                                                                    </td>
                                        ...
                                        <div class="tab-pane" id="info-detail" role="tabpanel">
                                            <div class="table-responsive">
                                                <table class="table w-100 table-sm" id="table-info" data-url=" <?= site_url('rcv/get_info/' . base64url_encode($this->encrypt->encode($data->TAG_ID))) ?>">
...
<div id="table-info-detail" class="d-none" data-url="<?= site_url('rcv/get_info_detail/') ?>">
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#statusTagId').html(badgeStatus(response.data[0].DISPLAY_NAME, response.data[0].MENU_ICON));
                $('#readonlyTagId').hide();

                if (response.data[0].ITEM_FLAG === 'N') {
                    $('#readonlyTagId').show();
                    $('#readonlyTagId').html('<span class="badge bg-secondary">READ ONLY</span>');

                    $('#myForm')
                        .find('input, select, textarea, #removeRow, #btn-modalSTS, td input')
                        .prop('disabled', true);
        ...
        if (oldDetail && oldDetail.kode_item) {
            oldDetail.kode_item.forEach(function(kode, i) {
                ...
                let rowNode = tableDetail.row.add([
                    ...
                    <input type="text" class="form-control form-control-sm qty edit-mode qty-edit text-end input-number d-none enter-as-tab w-100" name="detail[jumlah][]" value="${Number(jumlah)}" min="0" step="any" data-balance="${Number(balance)}">`,
