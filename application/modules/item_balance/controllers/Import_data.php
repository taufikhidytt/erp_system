...
class Import_data extends MX_Controller
{
    ...
    const HEADER_TEMPLATE = [
        'ITEM_CODE', 'ITEM_NAME', 'UOM_CODE', 'QTY_AWAL', 'HPP','SUBTOTAL'
    ];
    ...
    public function cli_process($encode_param, $encoded_db)
    {
        ...
        $warehouse = isset($param['warehouse'])?$param['warehouse']:[];
        $dt_period = isset($param['dt_period'])?$param['dt_period']:[];

        if(!$job_id || !isset($warehouse['WAREHOUSE_ID']) || !isset($dt_period['PERIOD_NAME'])){
        ...
        $this->load->model('Item_balance_import_model', 'import_model');
        ...
        try {
            ...
            while ($progress < $max_progress) {
                ...
                $lookups = $this->_preload_db($dataChunk, $warehouse, $dt_period);
            ...
            $this->import_model->update_job($job_id, [
                'STATUS'      => 'done',
                'MESSAGE'     => 'Validasi Gudang '.$warehouse['WAREHOUSE_NAME'].' periode '.$dt_period['PERIOD_NAME'].' telah selesai. ' . count($results['success']) . ' success, ' . count($results['failed']) . ' gagal.',
            ...
            $this->import_model->update_job($job_id, [
                'STATUS'      => 'failed',
                'MESSAGE'     => 'Gudang '.$warehouse['WAREHOUSE_NAME'].' periode '.$dt_period['PERIOD_NAME'].' Error: ' . $e->getMessage(),
                'FINISHED_AT' => date('Y-m-d H:i:s'),
                'PROCESS_ID'  => null,
            ]);
        }
    ...
    private function _validate_and_map_row(array $row, $row_number, array $lookups, array &$results)
    {
        ...
        $key_item = $this->_clean_key(trim($row['ITEM_CODE']).'_'.trim($row['ITEM_NAME']).'_'.trim($row['UOM_CODE']));
        $val_item = isset($lookups[$key_item])? (int) $lookups[$key_item]: 0;

        if (!$val_item)    $errors[] = 'Item tidak ditemukan di master data';
        ...
        foreach ([
            ...
            if((float) $val < $attr['min']){
                $errors[] = ucwords(str_replace('_',' ', $required[$field])).' minimal nilai adalah '.$attr['min'];
            }
        }

        // --- Cek duplikat ---
        if (empty($errors)) {
            if (isset($this->dt_items[$key_item])) {
                $errors[] = 'Item sudah '.($this->dt_description[$key_item] =='db'?'terdaftar di database':'ada pada baris sebelumnya');
            }else{
                $this->dt_items[$key_item] = 'file';
            }
        }
        // --- End cek duplikasi
        ...
        if (!empty($errors)) {
            ...
            $mapped['SUBTOTAL'] = (float) $mapped['QTY_AWAL'] * (float) $mapped['HPP'];
            $mapped['update'] = [
                'ITEM_BALANCE_ID'   => (int) $val_item,
            ];
            $results['success'][]   = $mapped;
        }
    }
    
    private function _preload_db(array $dataChunk, array $warehouse, array $dt_period)
    {
        $arr_item_code  = [];
 
        foreach ($dataChunk as $row) {
            $item_code  = trim($row['ITEM_CODE'] ?? '');
            if($item_code) $arr_item_code[] = $item_code;
        }
 
        return $this->import_model->get_item($arr_item_code, (int) $warehouse['WAREHOUSE_ID'], (int) $dt_period['PERIOD_NAME']);
    }
}