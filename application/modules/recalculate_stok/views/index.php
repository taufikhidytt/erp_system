                ...
                <div class="card rounded-3 p-4" id="card-sync-main">
                    ...
                    <div id="state-idle">
                        <div class="text-center py-3">
                            ...
                            <h5 class="fw-semibold mb-1">Sinkronisasi Item Stok</h5>
...
let xhr = null, xhr_history = null, xhr_delete = null;
...
function startSync() {
    Swal.fire({
            title: 'Yakin ingin proses sinkronisasi item stok?',
            icon: 'warning', showCancelButton: true, confirmButtonColor: '#70bcff', cancelButtonColor: '#d33', confirmButtonText: 'Ya, Lanjutkan!', cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                ...
                xhr = $.ajax({
                    url: '<?= base_url("recalculate_stok/start_sync") ?>',
                    type: 'POST',
                    dataType: 'json',
...
function pollProgress() {
    ...
    xhr = $.ajax({
        url: '<?= base_url("recalculate_stok/check_progress/") ?>' + currentJobId,
        type: 'GET',
        dataType: 'json',
        success: function(res) {
...
function cancelSync() {
    ...
    Swal.fire({
        ...
        icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#70bcff', confirmButtonText: 'Ya, batalkan!', cancelButtonText: 'Batal'
    }).then((result) => {
        if (result.isConfirmed) {
            if(xhr_delete){
                xhr_delete.abort();
            ...
            xhr_delete = $.ajax({
                url: '<?= base_url("recalculate_stok/cancel_sync") ?>',
                type: 'POST',
...
function history(){
    ...
    xhr_history = $.ajax({
        url: '<?= base_url("recalculate_stok/history") ?>',
        type: 'GET',
        dataType: 'json',
        success: function(res) {
            if (res.success) {
