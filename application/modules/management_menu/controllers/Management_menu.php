...
class Management_menu extends Back_Controller
{
    ...
    public function get_data()
    {
        ...
        foreach ($list as $m) {
            ...
            $row['no']      = $no;
            $row['seq']     = $m->SEQ;
            $row['name']    = '<a href="'.base_url('management_menu/'.($m->PARENT_ID ==0?'detail':'detail_child').'/'.base64url_encode($this->encrypt->encode($m->ERP_MENU_ID))).'">'.$name.'</a>';
            ...
            if($this->access['update']){
                $actions .= $m->PARENT_ID != 0 ? '<a href="'.base_url('management_menu/permissions/'.base64url_encode($this->encrypt->encode($m->ERP_MENU_ID))).'" class="btn btn-success btn-sm" title="Permissions" data-bs-toggle="tooltip" data-bs-placement="left"><i class="ri ri-key-fill"></i></a>' : '';
            }
            if($this->access['insert']){
                $actions .= $m->PARENT_ID == 0 ? '<a href="'.base_url('management_menu/add_child/'.base64url_encode($this->encrypt->encode($m->ERP_MENU_ID))).'" class="btn btn-primary btn-sm" title="Add Child" data-bs-toggle="tooltip" data-bs-placement="left"><i class="ri ri-add-fill"></i></a>' : '';
            }
            $row['action'] = $actions;
            $data[]     = $row;
        }
    ...
    public function add()
    {
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->template->load('template', 'management_menu/add', $data);
            } else {
                $this->db->trans_start();
                $post = $this->input->post();
                $params = [
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $data['data']       = $query->row();
                    $this->template->load('template', 'management_menu/detail', $data);
                } else {
                    $this->session->set_flashdata('warning', 'Data tidak ditemukan!');
                    redirect('management_menu');
                }
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $this->template->load('template', 'management_menu/add_child', $data);
                } else {
                    $this->session->set_flashdata('warning', 'Data tidak ditemukan!');
                    redirect('management_menu');
                }
                ...
                if ($query->num_rows() > 0) {
                    ...
                    if ($this->db->trans_status() === FALSE) {
                        ...
                        $error_msg = (ENVIRONMENT !== 'production') ? ' - ' . $db_error['message'] : '';
                        $this->session->set_flashdata('warning', 'Gagal menyimpan data!'. $error_msg);
                        redirect('management_menu/add_child/' . $encoded_id);
                    } else {
                        $this->session->set_flashdata('success', 'Selamat anda berhasil menyimpan data!');
                        redirect('management_menu/detail_child/' . $encoded_child_id);
                    }
                } else {
                    $this->session->set_flashdata('warning', 'Data tidak ditemukan!');
                    redirect('management_menu');
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($query->num_rows() > 0 && $query->row()->PARENT_ID) {
                    ...
                    $this->template->load('template', 'management_menu/detail_child', $data);
                } else {
                    $this->session->set_flashdata('warning', 'Data tidak ditemukan!');
                    redirect('management_menu');
                }
                ...
                if ($query->num_rows() > 0 && $query->row()->PARENT_ID) {
                    ...
                    if ($this->db->trans_status() === FALSE) {
                        ...
                        $error_msg = (ENVIRONMENT !== 'production') ? ' - ' . $db_error['message'] : '';
                        $this->session->set_flashdata('warning', 'Gagal menyimpan data!'. $error_msg);
                        redirect('management_menu/detail_child/' . $encoded_id);
                    } else {
                        $this->session->set_flashdata('success', 'Selamat anda berhasil menyimpan data!');
                        redirect('management_menu/detail_child/' . $encoded_id);
                    }
    ...
    public function sort()
    {
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->template->load('template', 'management_menu/sort', $data);
            } else {
