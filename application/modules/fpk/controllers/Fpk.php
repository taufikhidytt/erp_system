...
class Fpk extends Back_Controller
{
    ...
    public function get_log_info()
    {
        ...
        $config = [
            'queries' => [
                [
                    ...
                    'where'  => "a.TABLE_NAME = 'PR' AND a.ID = " . $id,
                ],
                [
                    'select' => 'a.LAST_UPDATE_DATE, u.ERP_USER_NAME, a.TRANSAKSI, a.NOTE',
                    'table'  => 'erp_log_edit a',
                    'join'   => ['erp_user u', 'a.LAST_UPDATE_BY = u.ERP_USER_ID'],
                    'where'  => "a.TABLE_NAME = 'PR_DETAIL' AND a.ORDER_ID = " . $id,
                ],
            ],
        ...
        $info_header    = $this->fpk->get_log_user($id);
        $result['header'] = [
    ...
    {
        $result = $this->fpk->getApiSales()->result();
        echo json_encode($result);
    }
}
