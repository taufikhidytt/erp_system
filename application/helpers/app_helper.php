...
if ( ! function_exists('numb_format'))
{
    function numb_format($value, $decimal = null, $thousand = INPUT_NUMBER_THOUSAND, $decimal_sep = INPUT_NUMBER_DECIMAL_SEP)
    {
        ...
        $num = numb_unformat($value, $thousand, $decimal_sep);
 
        if ($num === null) return '';
 
        if (is_nan($num)) $num = 0;
 
        $formatted = number_format($num, $decimal, $decimal_sep, $thousand);
    ...
    function checkAccess($action,$url=''){
        ...
        if(!isset($access[$action]) || !$access[$action]){
            if (strpos($ci->input->server('HTTP_ACCEPT'), 'application/json') !== false) {
                sendWarning('Anda tidak ada akses hapus untuk menu ini, silahkan hubungi administrator untuk mendapatkan akses tersebut!');die();
            } else {
    ...
    */
    function button_actions($arr, $type = 'html')
    {
        ...
        $template = [
            ...
            'print'  => ['icon' => 'ri-printer-fill',      'class' => 'btn-danger',    'class_custom' => '',           'title' => 'Print',   'target' => '_blank', 'needs_auth' => true],
            'delete' => ['icon' => 'ri-delete-bin-5-fill', 'class' => 'btn-danger',    'class_custom' => 'btn-delete', 'title' => 'Hapus',   'needs_auth' => true],
            'reload' => ['icon' => 'ri-reply-fill',        'class' => 'btn-warning',   'class_custom' => '',           'title' => 'Reload',  'onclick' => 'location.reload()', 'needs_auth' => false],
